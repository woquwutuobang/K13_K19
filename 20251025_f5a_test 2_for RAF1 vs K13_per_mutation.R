...
data_plot<-merge(merge_raf1,merge_k13,by=c("Pos_real","id","Pos"),suffixes = c(paste0("_", assay_sele2), paste0("_", assay_sele1)),all=F)
...
p <- ggplot(df, aes(x = residual_K13, y = residual_RAF1)) +
  ...
  labs(
    x = "Residual K13",
    y = "Residual RAF1",
    title = "Residual correlation between K13 and RAF1"
  ) +
  stat_cor(method = "pearson", label.x = min(df$residual_K13), label.y = max(df$residual_RAF1))
...
ggsave("C:/Users/36146/OneDrive - USTC/Manuscripts/K13_K19/figures/figure5/20251025/f5a Correlation of Residuals between K13 and RAF1.pdf",
