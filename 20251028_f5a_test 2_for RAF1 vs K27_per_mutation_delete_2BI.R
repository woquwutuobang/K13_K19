...
# K27 assay
result_k27 <- calculate_distance_residuals(
...
merge_raf1<-merge(ddG_raf1,result_raf1,by=c("Pos_real","mean_kcal/mol_RAF1"),all=F)

####
ddG_k27<-fread("C:/Users/36146/OneDrive - USTC/DryLab/MoCHI_8binders_l2_e6_RA_old_new_merge_at_mochi_20250901/task_901/weights/weights_Binding_K27.txt")
names(ddG_k27)
ddG_k27<-ddG_k27[,Pos_real:=Pos+1]
ddG_k27 <- ddG_k27[, c(1,20,23)]
colnames(ddG_k27)[2] <- paste0(colnames(ddG_k27)[2], "_", assay_sele2)

setnames(result_k27, "original_y",  paste0("mean_kcal/mol_", assay_sele2))

merge_k27<-merge(ddG_k27,result_k27,by=c("Pos_real","mean_kcal/mol_K27"),all=F)

names(merge_raf1)

data_plot<-merge(merge_raf1,merge_k27,by=c("Pos_real","id","Pos"),suffixes = c(paste0("_", assay_sele2), paste0("_", assay_sele1)),all=F)
...
# 在绘图前过滤掉结合界面位点
data_plot_filtered <- data_plot[!Pos_real %in% c(K27_Binding_interface_site, RAF1_Binding_interface_site)]

cat("过滤前数据点:", nrow(data_plot), "\n")
cat("过滤后数据点:", nrow(data_plot_filtered), "\n")
cat("排除的结合界面位点数量:", length(c(K27_Binding_interface_site, RAF1_Binding_interface_site)), "\n")
...
ggsave("C:/Users/36146/OneDrive - USTC/Manuscripts/K13_K19/figures/figure5/20251028/f5a Correlation of Residuals between K27 and RAF1 (excluded interface sites).pdf",
